...
Feature: SpecListItem Component
  ...
  Scenario: Renders as a dense row with all metadata
    Given a SpecListItem with title "Context Gates", status "Live", and tags ["Architecture"]
  ...
  Scenario: Stacks without spacing conflicts
    Given multiple SpecListItems are rendered in a flex column
  ...
  Scenario: Renders gracefully with optional props omitted
    Given a SpecListItem with only title and url provided
    When it is rendered
